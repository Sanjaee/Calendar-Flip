	...
	behavior:function(){
		...
		$bookStage.swipeMePls({
		    ...
		    onSwipe:function(opt){              
			  // Flip pages with vertical swipe (up/down) instead of horizontal
			  if(opt.direction==='up' || opt.direction==='down'){
				var go = opt.direction==='up'?'gotoNext':'gotoPrev';
		        _this.clickSpeedIsOk()&&_this[go]();
			  }else if(opt.direction==='right' || opt.direction==='left'){
				// Use horizontal swipe to change slope/tilt
				var tiltDir = opt.direction==='right' ? 'up' : 'down';
				_this.slopeAnimateTo(tiltDir);
			  }
			}
		});
		...
		.mousemove(function(e){
			...
			if(_this.startDrag){
				...
				var mouseSpeedY = Math.abs(_this.MOUSE.oldCoord.y-_this.MOUSE.lastCoord.y);
				if(mouseSpeedY > _this.CFG.MAX_MOUSE_SPEED_X){
					_this.stop_drag_and_animate();
					return;
				};

				var deltaY = Math.abs(_this.MOUSE.startDragCoord.y - _this.MOUSE.lastCoord.y);
				var drag_persent = Math.round(deltaY/_this.CFG.DRAG_PARAM.px_per_persent);
